...
func (c *Conn) handleMail(arg string) {
    ...
    // Generate a queue id, and put it to opts

    c.queueId = generateQueueID()
    opts.QueueId = c.queueId
    ...
    if c.queueId == "XXX" {
        c.writeResponse(451, EnhancedCode{5, 5, 4}, "Internal error creating a queue ID")
        return
    }
